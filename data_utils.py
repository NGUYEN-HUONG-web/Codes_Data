...
def tokenize(text: str):
    """Tokenize bằng cách tách theo khoảng trắng và chuyển về chữ thường.

    Trả về danh sách token từ `text`.
    """
    return text.strip().lower().split()


def build_vocab(df: pd.DataFrame):
    """Xây dựng từ vựng từ dataframe train.

    Trả về dict mapping token -> id, với `PAD_TOKEN` = 0 và `UNK_TOKEN` = 1.
    """
    counter = Counter()
    for text in df["text"]:
...
def encode_text(text: str, vocab: dict[str, int], max_len: int):
    """Chuyển `text` thành danh sách id có độ dài cố định.

    Trả về (ids, length) trong đó `ids` được padding/ truncation tới `max_len`,
    `length` là số token thực tế (bị giới hạn bởi `max_len`).
    """
...
def dataframe_to_tensor_dict(df: pd.DataFrame, vocab: dict[str, int], max_len: int):
    """Chuyển dataframe thành dict có thể lưu gồm tensor và văn bản gốc.

    Trả về dict chứa `input_ids`, `lengths`, `labels`, `texts`. `input_ids`,
    `lengths`, `labels` là PyTorch tensors sẵn cho DataLoader.
    """

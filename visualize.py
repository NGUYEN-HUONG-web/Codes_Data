...
def tokenize(text: str):
    """Tokenize đơn giản dùng cho visualize (lower + split theo khoảng trắng)."""
    return text.strip().lower().split()


def encode_text(text: str, vocab: dict[str, int], max_len: int):
    """Mã hóa câu thành id theo `vocab` và trả về danh sách token.

    Trả về `(ids, tokens)` trong đó `ids` được padding/truncate tới `max_len`,
    còn `tokens` dùng để gán nhãn trục của heatmap.
    """
...
def pick_sentence_from_test(processed_dir: Path):
    """Lấy câu đầu tiên trong `test.pt` làm ví dụ mặc định."""

...
def scaled_dot_product_attention(Q, K, V):
    ...
    Với Q, K, V, hàm tính ma trận điểm (scores), áp dụng softmax
    và trả về vector ngữ cảnh (output) cùng ma trận trọng số attention.

    Tham số:
        Q, K, V: tensor có dạng (batch_size, seq_len, d_k)

    Trả về:
        output: tensor (batch_size, seq_len, d_k) -- vector ngữ cảnh
        weights: tensor (batch_size, seq_len, seq_len) -- ma trận attention
    """
    d_k = Q.size(-1)
    ...
    # apply softmax on last dim (keys dimension)
...
class PositionalEncoding(nn.Module):
    ...
    def forward(self, x):
        """Thêm positional encoding (sinusoidal) vào tensor đầu vào.

        `x` có dạng (batch_size, seq_len, d_model). Trả về tensor cùng kích
        thước với thông tin vị trí đã cộng vào.
        """
        seq_len = x.size(1)
...
class SelfAttention(nn.Module):
    ...
    def forward(self, x):
        """Tính self-attention cho đầu vào `x`.

        Chiếu `x` sang Q, K, V, áp dụng scaled dot-product attention
        và trả về vector ngữ cảnh cùng ma trận attention.
        """
        Q = self.q_proj(x)
        K = self.k_proj(x)
        V = self.v_proj(x)
...
class FeedForwardNetwork(nn.Module):
    ...
    def forward(self, x):
        """Áp dụng Feed-Forward Network hai lớp theo vị trí.

        `x` có dạng (batch_size, seq_len, d_model). Trả về tensor cùng kích thước.
        """
        x = self.fc1(x)
        x = F.relu(x)
        x = self.fc2(x)
...
class TransformerEncoderBlock(nn.Module):
    ...
    def forward(self, x):
        """Lượt truyền qua một Transformer encoder block.

        Các bước:
        1) Self-attention, cộng residual và LayerNorm
        2) Feed-forward, cộng residual và LayerNorm
        Trả về output và ma trận attention từ self-attention.
        """
...
class ClassifierHead(nn.Module):
    ...
    def forward(self, x):
        """Lấy trung bình theo chiều sequence rồi áp dụng lớp linear cuối cùng."""
        pooled = x.mean(dim=1)
